            ...
            if(assignObj && typeof assignObj === 'object'){
                for(let key in assignObj){
                    if( copySource[key] === undefined ){
                        if(typeof assignObj[key] === 'object'){
...
class WhiteboardIntermediateLayer{
    ...
    handlerPubmsg_SharpsChange(pubmsgData){
        if(pubmsgData){
            ...
            if(pubmsgData && pubmsgData.data != null && (pubmsgData.data.eventType === "shapeSaveEvent"
                || pubmsgData.data.eventType === "clearEvent" || pubmsgData.data.eventType === "redoEvent" || pubmsgData.data.eventType === "laserMarkEvent"  )) {
                pubmsgData.source = 'pubmsg';
        ...
        if(delmsgData){
            ...
            if(!whiteboardInstance){
                ...
                for(let i = this.awaitSaveToWhiteboardInstanceSignallingArray.length-1 ; i>=0 ; i--){
                    let awaitSaveToWhiteboardInstanceSignalling =  this.awaitSaveToWhiteboardInstanceSignallingArray[i] ;
            ...
            if(shapeName){
                ...
                if( whiteboardInstance.waitingProcessShapeData[shapeName] && Array.isArray(whiteboardInstance.waitingProcessShapeData[shapeName]) && whiteboardInstance.waitingProcessShapeData[shapeName].length > 0 ){
                    for(let i = whiteboardInstance.waitingProcessShapeData[shapeName].length-1 ; i>=0 ; i--){
                        let waitingProcessShape =  whiteboardInstance.waitingProcessShapeData[shapeName][i] ;
                        if(waitingProcessShape.id === delmsgData.id){
                            whiteboardInstance.waitingProcessShapeData[shapeName].splice(i , 1);
                            break ;
                        }
                    }
                }
    ...
    handlerMsglist_SharpsChange(sharpsChangeArray ){
        ...
        if(needLoadCurrentPageList && Object.keys(needLoadCurrentPageList).length){
            for( let id of Object.keys(needLoadCurrentPageList) ){
...
//      whiteboardInstance.lc.on("lc-pointerdrag",whiteboardInstance.lcPointerdragEvent) ;
    ...
    /*改变白板的专属属性*/
    changeWhiteboardExclusiveAttribute(id ,changeExclusiveAttribute){
        if(changeExclusiveAttribute && typeof changeExclusiveAttribute === 'object'){
            let whiteboardInstance = this._getWhiteboardInstanceById(id);
            if(!whiteboardInstance){L.Logger.error( '[changeWhiteboardExclusiveAttribute]There are no white board Numbers that belong to id '+id ) ;return ;}
            for(let key in changeExclusiveAttribute){
    ...
    /*更新提示内容显示权限*/
    updateShowRemindContent(id , showShapeAuthor , showMyselfRemindContent){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
    ...
    updateWhiteboardWatermarkImageScale(id , watermarkImageScale = this.defaultProductionOptions.watermarkImageScale ){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
        if(!whiteboardInstance){L.Logger.error( '[updateWhiteboardWatermarkImageScale]There are no white board Numbers that belong to id '+id ) ;return ;};
        this._updateWhiteboardWatermarkImageScale( whiteboardInstance , watermarkImageScale );
    }

    /*设置白板的图片*/
    setWhiteboardWatermarkImage(id ,watermarkImageUrl , {resetDedaultWhiteboardMagnification = true , loadSucceedCallback , loadFailureCallback } = {} ){
        const that = this ;
        ...
        if(!watermarkImageUrl){ //图片地址没有，则使用默认白板
            ...
            whiteboardInstance.setWhiteboardWatermarkImageTimer =  setTimeout(function(){
                ...
                watermarkImage.onload = function(){
                    if( !whiteboardInstance.currentWatermarkImageUrl ){ //HACK 此处处理动态PPT和H5课件不响应图片加载完后更改白板比例
                        // whiteboardInstance.watermarkImageScale = whiteboardInstance.defaultWhiteboardScale ;
                        whiteboardInstance.lc.watermarkImage = null ;
                        that._resizeWhiteboardHandler();
                    }else {
                        let watermarkImageScale =  watermarkImage.width / watermarkImage.height ;
                        ...
                        that._resizeWhiteboardByScale(whiteboardInstance , { watermarkImage, watermarkImageScale } );
        ...
        if( colorJson && typeof colorJson === 'object'){
            ...
            for(let [key ,value] of Object.entries(colorJson) ){
                /*
                 ...
                 backgroundColor:"#ffffff" , //背景颜色
                 */
                let colorKey =  key.replace(/Color/g , '');
    ...
    getActiveTool(id){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
        if(!whiteboardInstance){L.Logger.error( '[getActiveTool]There are no white board Numbers that belong to id '+id ) ;return ;}
    ...
    activeWhiteboardTool(toolKey , id){
        ...
        if( /^tool_/.test(toolKey) ){ //白板底层工具
            ...
            if(toolKey === 'action_undo'){
                let toAuthorUserId = whiteboardInstance.isOnlyUndoRedoClearMyselfShape && whiteboardInstance.myselfId ?  whiteboardInstance.myselfId : undefined ;
                ...
                let toAuthorUserId = whiteboardInstance.isOnlyUndoRedoClearMyselfShape && whiteboardInstance.myselfId ?  whiteboardInstance.myselfId : undefined ;
                if(whiteboardInstance.synchronizationWhiteboard){
                    whiteboardInstance.lc.sendRedo(toAuthorUserId);
                }else{
                ...
                let toAuthorUserId = whiteboardInstance.isOnlyUndoRedoClearMyselfShape && whiteboardInstance.myselfId ?  whiteboardInstance.myselfId : undefined ;
                if(whiteboardInstance.synchronizationWhiteboard){
                    whiteboardInstance.lc.sendClear(toAuthorUserId);
                }else{
    ...
    /*使用历史的画笔工具*/
    useHistoryPenTool(id){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
        if(!whiteboardInstance){L.Logger.error( '[useHistoryPenTool]There are no white board Numbers that belong to id '+id ) ;return ;}
        let toolKey = whiteboardInstance.historyPenTool || 'tool_pencil';
        this.activeWhiteboardTool(toolKey , id);
    }

    /*使用历史的画笔工具*/
    useHistoryShapeTool(id){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
        if(!whiteboardInstance){L.Logger.error( '[useHistoryShapeTool]There are no white board Numbers that belong to id '+id ) ;return ;}
        let toolKey = whiteboardInstance.historyShapeTool || 'tool_rectangle';
        this.activeWhiteboardTool(toolKey , id);
    }
    ...
    /*更新白板的属性*/
    updateWhiteboardProperty(id , updateProperty){
        const that = this ;
        if(updateProperty && typeof updateProperty === 'object'){
            let whiteboardInstance = this._getWhiteboardInstanceById(id);
            if(!whiteboardInstance){L.Logger.error( '[updateWhiteboardProperty]There are no white board Numbers that belong to id '+id ) ;return ;}
            ...
            if(updateProperty.primary !== undefined || updateProperty.secondary !== undefined || updateProperty.background !== undefined ||
                updateProperty.primaryColor !== undefined || updateProperty.secondaryColor !== undefined || updateProperty.backgroundColor !== undefined ){
                let updateColor = {};
                for(let colorKey in updateProperty){
                    if(colorKey === 'primary' || colorKey === 'secondary'  || colorKey === 'background' ||
                        colorKey === 'primaryColor' || colorKey === 'secondaryColor'  || colorKey === 'backgroundColor' ){
    ...
    hasWhiteboardFiledata(id){
        ...
        if(whiteboardInstance.filedata && typeof whiteboardInstance.filedata  === 'object' && Object.keys(whiteboardInstance.filedata).length > 0 ){
            return true ;
        }
        return false ;
    };
    ...
    /*获取注册的标注工具的描述*/
    getRegisterWhiteboardToolsList(id){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
        if(!whiteboardInstance){L.Logger.error( '[getRegisterWhiteboardToolsList]There are no white board Numbers that belong to id '+id ) ;return ;}
    ...
    /*清除指定文件和页数的画笔数据*/
    clearWhiteboardDataByFileidAndCurrpage(id , fileid , currpage){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
        if(!whiteboardInstance){L.Logger.error( '[clearWhiteboardDataByFileidAndCurrpage]There are no white board Numbers that belong to id '+id ) ;return ;}
    ...
    /*加载当前页的白板数据*/
    loadCurrpageWhiteboard(id , {loadRedoStack=true, callback} = {} ){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
    ...
    /*获取白板工具信息*/
    getWhiteboardToolsInfo( id  ){
        let whiteboardInstance = this._getWhiteboardInstanceById(id);
        if(!whiteboardInstance){L.Logger.error( '[getWhiteboardToolsInfo]There are no white board Numbers that belong to id '+id ) ;return ;}
   ...
   /* 接收白板数据操作shape画图*/
                ...
                switch(remoteData.data.eventType){
                    case "shapeSaveEvent":
                        if( whiteboardInstance && whiteboardInstance.lc && whiteboardInstance.lc.undoStack && whiteboardInstance.lc.undoStack.length > 0 ){
                            let isBreak = false ;
                            for(let i = whiteboardInstance.lc.undoStack.length -1 ; i>=0 ; i--){
                                let value = whiteboardInstance.lc.undoStack[i];
                    ...
                    case "redoEvent":
                        if(remoteData.data.actionName && remoteData.data.actionName === "AddShapeAction"){
                            let isRedo = true ;
                            for (let i= whiteboardInstance.lc.undoStack.length-1 ; i>=0 ; i-- ) {
                                if( remoteData.data.shapeId === whiteboardInstance.lc.undoStack[i].shapeId ){
                                    isRedo = false ;
                                    break ;
                                }
                            }
                            if(isRedo){
                                ...
                                if(flag){
                                    ...
                                    let {otherInfo = {authorUserId:remoteData.fromID  , seq:remoteData.seq } } = remoteData.data ;
                            ...
                            let isRedo = true ;
                            for (let i= whiteboardInstance.lc.undoStack.length-1 ; i>=0 ; i-- ) {
                                if(remoteData.data.clearActionId === whiteboardInstance.lc.undoStack[i].id ){
                                    isRedo = false ;
                                    break ;
                                }
                            }
                            if(isRedo){
                                let flag = false ;  //恢复栈中是否有该shape
                                for (let i= whiteboardInstance.lc.redoStack.length-1 ; i>=0 ; i-- ) {
                                    if( remoteData.data.clearActionId === whiteboardInstance.lc.redoStack[i].id ){
                                        flag = true ;
                                        break ;
                                    }
                                }
                                if(flag){
                                    ...
                                    let {otherInfo = {authorUserId:remoteData.fromID , toAuthorUserId:undefined , seq:remoteData.seq } } = remoteData.data ;
                                    whiteboardInstance.lc.clear(false ,  remoteData.data.clearActionId  , otherInfo.authorUserId ,  otherInfo.toAuthorUserId  , otherInfo.seq);
                                    this._removeAllRemindContent(whiteboardInstance);
                                }
                            }
                        }
    ...
    _updateToolDesc(whiteboardInstance , toolKey , toolValue){
        ...
        for(let key of Object.keys(whiteboardInstance.registerWhiteboardToolsList) ){
            if( /tool_/g.test(key) ){
   ...
   /*获取白板实例id,根据id获取*/
    ...
    /*白板大小根据比例自适应*/
    _resizeWhiteboardByScale(whiteboardInstance  , { watermarkImage , isChangeCanvas=true , isChangeWatermarkScale = true , watermarkImageScale} = {} ){
        ...
        if(whiteboardInstance_lc){
            ...
            if(isChangeCanvas){
                ...
                let whiteboardScale =  ( eleWidth + eleHeight  ) /  ( whiteboardInstance.baseWhiteboardWidth + whiteboardInstance.baseWhiteboardWidth*watermarkImageScale ) ;
                ...
                if(isChangeWatermarkScale && watermarkImage ){
                    ...
                    let lvW = eleWidth / watermarkImageWidth ;
                    let lvH =  eleHeight / watermarkImageHeight ;
                    let lv = (lvW+lvH)/2;
    ...
    /*更新白板的watermarkImageScale*/
    _updateWhiteboardWatermarkImageScale(whiteboardInstance , watermarkImageScale){
    ...
    _handlerActiveToolLaser( toolKey , whiteboardInstance){
        ...
        const _handlerActiveToolLaserFromInner = (whiteboardInstance)=> {
            ...
            if(toolKey === 'tool_laser'){
                ...
                whiteboardInstance._laserEventHandler_mousemove =   ( (e) => {
                ...
                whiteboardInstance._laserEventHandler_mouseenter =  ( (e) => {
                        if(whiteboardInstance.activeTool  !== 'tool_laser'){
                            let parameter = {
                                action:{
                                    actionName:"hide"
                ...
                whiteboardInstance._laserEventHandler_mouseleave =  ( (e) => {
                        let parameter = {
                            action:{
                                actionName:"hide"
                            }
    ...
    /*发送白板数据信令给服务器*/
    _sendSignallingToServer(whiteboardInstance , {idPrefix , data , name , assignId , do_not_save , expiresabs  , associatedMsgID , associatedUserID } = {}){
        if(!whiteboardInstance){
        ...
        if(whiteboardInstance.handler && whiteboardInstance.handler.sendSignallingToServer && typeof whiteboardInstance.handler.sendSignallingToServer === 'function' ){
            ...
            let id = assignId || idPrefix + "###_"+name+"_"+fileid+"_"+currpage , toID = "__allExceptSender" ;
            if(data.eventType === 'shapeSaveEvent' || data.eventType === 'undoEvent' || data.eventType === 'redoEvent' || data.eventType === 'clearEvent' ){
                toID = '__all' ;
            }
            let copyData = Object.deepAssign({} , data );
            if( toID !== '__all' ){
                this._saveBasicTemplateWhiteboardSignallingData({name ,id , toID ,  data:copyData , do_not_save , expiresabs  , associatedMsgID , associatedUserID} , 'pubmsg' );
            }
    ...
    /*发送白板数据信令给服务器*/
    _delSignallingToServer(whiteboardInstance , { idPrefix , data , name , assignId  } = {}){
        if(!whiteboardInstance){
        ...
        if(whiteboardInstance.handler && whiteboardInstance.handler.delSignallingToServer && typeof whiteboardInstance.handler.delSignallingToServer === 'function'){
            ...
            let copyData = Object.deepAssign({} , data );
            if( toID !== '__all' ) {
                this._saveBasicTemplateWhiteboardSignallingData({name, id, toID, data: copyData}, 'delmsg');
            }
    ...
    _sendWhiteboardMessageToSignallingServer( whiteboardInstance ,  eventType , parameter ){
        const that = this ;
        let idPrefix , data  , name   , assignId , do_not_save  , shapeData , testData;
        switch(eventType){
            case "shapeSaveEvent":
                ...
                that._sendSignallingToServer(whiteboardInstance , {idPrefix , data , name , assignId , do_not_save} );
                break;
            case "undoEvent" :
                if( parameter.action.actionName === "AddShapeAction" ){
                    ...
                    that._delSignallingToServer(whiteboardInstance , { idPrefix , data , name , assignId } );
                    ...
                    idPrefix = clearActionId  , data =  testData , name = "SharpsChange"  , assignId = undefined ;
                    that._delSignallingToServer(whiteboardInstance , { idPrefix , data , name , assignId } );
                }
                break ;
            case "redoEvent" :
                if( parameter.action.actionName === "AddShapeAction" ){
                    ...
                    let shapeId =  parameter.action.shapeId ;
                    let otherInfo = {authorUserId: parameter.action.authorUserId , seq: parameter.action.seq };
                    testData  = {eventType:eventType  , actionName:parameter.action.actionName  , shapeId:shapeId ,  data:shapeData , otherInfo:otherInfo };
                    let idPrefix = shapeId  , data =  testData , name = "SharpsChange"  , assignId = undefined , do_not_save = undefined ;
                    that._sendSignallingToServer(whiteboardInstance , {idPrefix , data , name , assignId , do_not_save} );
                }else if(parameter.action.actionName === "ClearAction"){
                    let clearActionId = parameter.action.id ;
                    let otherInfo = {authorUserId: parameter.action.authorUserId , seq: parameter.action.seq , toAuthorUserId:parameter.action.toAuthorUserId};
                    testData  = {eventType:eventType , actionName:parameter.action.actionName , clearActionId:clearActionId  , otherInfo:otherInfo};
    ...
    _updateTextFont( whiteboardInstance){
        ...
        const _updateTextFontFromInner = (whiteboardInstance) => {
            let fontSize = whiteboardInstance.whiteboardToolsInfo.fontSize ;
            let fontFamily = whiteboardInstance.whiteboardToolsInfo.fontFamily ;
            let fontStyle = whiteboardInstance.whiteboardToolsInfo.fontStyle ;
            let fontWeight = whiteboardInstance.whiteboardToolsInfo.fontWeight ;
            ...
            const _updateColorFromInner = (whiteboardInstance) => {
                for(let key of Object.keys(colorJson) ){
                    ...
                    if(  ( colorKey==="primary" || colorKey==="secondary" ) && ( whiteboardInstance.activeTool === "tool_rectangle" || whiteboardInstance.activeTool === "tool_rectangle_empty" || whiteboardInstance.activeTool === "tool_ellipse" || whiteboardInstance.activeTool === "tool_ellipse_empty"   || whiteboardInstance.activeTool === "tool_polygon"  ) ){
                        if( whiteboardInstance.activeTool === "tool_ellipse_empty"  || whiteboardInstance.activeTool === "tool_rectangle_empty" ){ //空心
                            whiteboardInstance.lc.setColor('secondary',"transparent" ) ;
                        }else{
                            whiteboardInstance.lc.setColor('secondary',whiteboardInstance.whiteboardToolsInfo.primaryColor);
                        }
                    ...
                    if( colorKey==="primary" && whiteboardInstance.activeTool === 'tool_highlighter' ){
                        let color = whiteboardInstance.whiteboardToolsInfo[colorKey+"Color"].colorRgb().toLowerCase().replace("rgb","rgba").replace(")",",0.5)") ;
            ...
            if(whiteboardInstance.active){ //如果白板处于激活动态
                if(whiteboardInstance.lc.shapes.length === 0 ){ //白板没有画笔数据
                    ...
                    if(whiteboardInstance.isOnlyUndoRedoClearMyselfShape && whiteboardInstance.myselfId){
                        ...
                        for(let i = 0 ; i<whiteboardInstance.lc.shapes.length ; i++){
    ...
    /*通知白板工具信息更新*/
    _noticeNpdateWhiteboardToolsInfo(whiteboardInstance){
...
const  WhiteboardIntermediateLayerInstance = new WhiteboardIntermediateLayer();
// window.WhiteboardInstance = WhiteboardIntermediateLayerInstance ;
export default WhiteboardIntermediateLayerInstance ;
